...
{
    PT_SEMICOLON, PT_END, PT_ELSE, PT_UNTIL, PT_DOT, PT_ALLOW, PT_OTHERWISE,
};
...
ICodeNode *StatementParser::parse_statement(Token *token) 
{
    ...
    switch ((PascalTokenType) token->get_type())
    {
        ...
        default:
        {
            statement_node = ICodeFactory::create_icode_node((ICodeNodeType) NT_NO_OP);
            break;
        }
    }
